...
def main():
    ...
    # 9. Call LLM to get an expert-style summary (if keys available)
    try:
        from llm_agent import summarize_insights
        analysis_text = []
        # reuse previous printed insights where helpful
        analysis_text.append('PCA explained variance: ' + ', '.join([f"PC{i+1}:{v:.2%}" for i, v in enumerate(pca.explained_variance_ratio_)]))
        analysis_text.append('\nCluster summary:\n' + cluster_stats.to_csv())
        analysis_text.append('\nCorrelation matrix:\n' + correlation_matrix.to_csv())
        analysis_blob = '\n'.join(analysis_text)
        print('\nRequesting LLM summary (OpenAI/HF fallback if configured)...')
        llm_out = summarize_insights(analysis_blob)
        print('\n=== LLM-style Summary and Recommendations ===')
        print(llm_out)
    except Exception as e:
        print('LLM summarization not available:', e)
